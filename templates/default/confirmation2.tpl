						...
						{if $Sytempay_enable>0 }
							{if {$total}>0 }
								<form method="post" action="{$url_payment_systempay}">
									...
									<input type="hidden" name="vads_order_info" value="GA|{foreach from=$activities key=k item=activity}{if $followups[$k]->statut_act == 1}756_{$activity->id_group}_{if $category == 0}{$activity->price1*100|escape|nl2br}{/if}{if $category == 1}{$activity->price2*100|escape|nl2br}{/if}/{/if}{/foreach}|">
